...
from config.settings import NOTIFICATION_SERVICE_PATH_EMAIL as email_path, NOTIFICATION_SERVICE_PATH_WEBSOCKET as \
    websocket_path, NOTIFICATION_SERVICE_PATH_WEBPUSH as webpush_path
from notification.models import MessageTemplate
from notification.services import DummyMailGenerator, send_notify, WebsocketGenerator, WebpushGenerator
...
def friday_top_email():
    ...
    email_template: MessageTemplate = MessageTemplate.objects.filter(
        mail_type='selection_movies', channel='email').first()
...
def friday_top_websocket():
    ...
    web_template: MessageTemplate = MessageTemplate.objects.filter(
        mail_type='selection_movies', channel='websocket').first()
...
def friday_top_webpush():
    ...
    web_template: MessageTemplate = MessageTemplate.objects.filter(
        mail_type='selection_movies', channel='web_push').first()
